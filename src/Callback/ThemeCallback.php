...
class ThemeCallback
{
    ...
    public function initialize($dataProvider)
    {
        ...
        if (!($controller = $service->getDataProviderController('tl_theme'))
    ...
    public function replaceRowIdWithSerializedModelId(
        ...
        $modelId = new ModelId($strTable, $row['id']);

        switch ($icon) {
            case 'css.gif':
                $row['id'] = $modelId->getSerialized();
                return $user->hasAccess('css', 'themes') ? '<a href="'.Controller::addToUrl($href.'&amp;pid=' . $row['id']) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon, $label) . '</a> ' : Image::getHtml(preg_replace('/\.gif$/i', '_.gif', $icon)) . ' ';

                break;

            case 'modules.gif':
                $row['id'] = $modelId->getSerialized();
                return $user->hasAccess('modules', 'themes') ? '<a href="'.Controller::addToUrl($href.'&amp;pid=' . $row['id']) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon, $label) . '</a> ' : Image::getHtml(preg_replace('/\.gif$/i', '_.gif', $icon)) . ' ';

                break;

            case 'layout.gif':
                $row['id'] = $modelId->getSerialized();
                return $user->hasAccess('layout', 'themes') ? '<a href="'.Controller::addToUrl($href.'&amp;pid=' . $row['id']) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon, $label) . '</a> ' : Image::getHtml(preg_replace('/\.gif$/i', '_.gif', $icon)) . ' ';
            ...
            case 'sizes.gif':
                $row['id'] = $modelId->getSerialized();
                return $user->hasAccess('image_sizes', 'themes') ? '<a href="'.Controller::addToUrl($href.'&amp;pid=' . $row['id']) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon, $label) . '</a> ' : Image::getHtml(preg_replace('/\.gif$/i', '_.gif', $icon)) . ' ';

                break;


            case 'theme_export.gif':
                return $user->hasAccess('theme_export', 'themes') ? '<a href="'.Controller::addToUrl($href.'&amp;pid=' . $row['id']) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon, $label) . '</a> ' : Image::getHtml(preg_replace('/\.gif$/i', '_.gif', $icon)) . ' ';
